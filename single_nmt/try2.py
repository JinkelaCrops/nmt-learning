...
def get_iterator(src_dataset, tgt_dataset,
    ...
    src_eos_id = tf.cast(src_vocab_table.lookup(tf.constant(eos)),
                         tf.int32)  # 转换 https://tensorflow.google.cn/versions/r1.7/api_docs/python/tf/cast
...
def create_train_model(model_creator, hparams, scope=None, num_workers=1, jobid=0, extra_args=None):
    ...
    with graph.as_default(), tf.container(scope or "train"):
        src_vocab_table, tgt_vocab_table = create_vocab_tables(src_vocab_file, tgt_vocab_file,
                                                               hparams.share_vocab)  # 2个vocab, https://tensorflow.google.cn/versions/r1.7/api_docs/python/tf/contrib/lookup/index_table_from_file

        src_dataset = tf.data.TextLineDataset(
            src_file)  # https://tensorflow.google.cn/versions/r1.7/api_docs/python/tf/data/TextLineDataset
...
# Embeddings
embedding_encoder, embedding_decoder = create_emb_for_encoder_and_decoder(
    share_vocab=hparams.share_vocab,
